...
class UserController extends Controller
{
    ...
    public function showAuthorizationList()
    {
        ...
        return view('BlessingSkin\\OAuth::user.authorizations', [
    ...
    public function revokeAuthorization($id)
    {
        ...
        // 撤销授权码
        \BlessingSkin\OAuth\AuthCode::where('user_id', $userId)
            ->where('client_id', $clientId)
            ->update(['revoked' => true]);
        
        // 撤销访问令牌
        \BlessingSkin\OAuth\Token::where('user_id', $userId)
            ->where('client_id', $clientId)
            ->update(['revoked' => true]);
        ...
        return response()->json([
            'code' => 0,
            'message' => trans('BlessingSkin\\OAuth::user.authorization-revoked'),
        ]);
    }
}
